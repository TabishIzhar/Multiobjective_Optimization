# Beam 2: Design optimization of RC beam 2 under simple optimization.
...
t1= time.time()
for i in range (1):
    r1= rcb(width=300, depth=450, length=5, bending_moment=[240.5, 100.16, 223.68], shear_force=[159.79, 99.24, 150.92], cover=30, left_end_continous= True)      #Creating beam object
...
with pd.ExcelWriter('./Result_Beam2_WC_df.xlsx',) as writer:
     for i in range (1):
        dff[i].to_excel(writer, sheet_name=f'Opti{i+1}')


with pd.ExcelWriter('./Result_Beam2_WC_rd.xlsx',) as writer:
     for i in range (1):
...
with pd.ExcelWriter('./Result_Beam2_WC_sd.xlsx',) as writer:
     for i in range (1):
