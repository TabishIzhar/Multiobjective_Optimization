...
t1=time.time()
for i in range (1):
    r1= rcb(width=300, depth=450, length=4, bending_moment=[138.26, 64.07, 138.26], shear_force=[121.55, 76.94, 121.55], cover=30)     #Creating beam object
...
t2=time.time()
comp_time= t2-t1
with pd.ExcelWriter('./Result_Beam1_WC_df.xlsx',) as writer:
     for i in range (1):
        dff[i].to_excel(writer, sheet_name=f'Opti{i+1}')


with pd.ExcelWriter('./Result_Beam1_WC_rd.xlsx',) as writer:
     for i in range (1):
...
with pd.ExcelWriter('./Result_Beam1_WC_sd.xlsx',) as writer:
     for i in range (1):
